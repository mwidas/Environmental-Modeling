...
thresh_cap = 0.5*sens_P$X[1,"K"]
idx_cap = which(result$P > thresh_cap)[1]

# if it never gets there
idx_cap = ifelse(is.na(idx_cap), length(result$P), idx_cap)
# turn this index into a year (might be the same if time step in 1 but just in case it isn't)
threshyear_cap = result$time[idx_cap]
threshyear_cap

ggplot(result, aes(time, P))+geom_line()+geom_vline(xintercept=threshyear_cap, col="red")+labs(y="Population",title="When do we get to 50%\nof carrying capacity")

# or how about the population at year 5
thresh_pop = 5
idx_pop = which(result$time > thresh_pop)[1]

# if it never gets there
idx_pop = ifelse(is.na(idx_pop), length(result$P), idx_pop)
# turn this index into a year (might be the same if time step in 1 but just in case it isn't)
threshyear_pop = result$time[idx_pop]
threshyear_pop

ggplot(result, aes(time, P))+geom_line()+geom_vline(xintercept=threshyear_pop, col="red")+labs(y="Population",title="What population is at year 5")
```
